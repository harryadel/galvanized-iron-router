...
function safeDecodeURIComponent (val) {
  try {
    ...
    if (e.constructor == URIError) {
      warn(false, "Tried to decode an invalid URI component: " + JSON.stringify(val) + " " + e.stack);
    }
...
/**
 * Form encoding treats '+' as a space. That convention applies to query
 * strings only, never to path segments, where '+' is a literal character
 * (RFC 3986).
 */
function safeDecodeQueryComponent (val) {
  return safeDecodeURIComponent(val.replace(/\+/g, ' '));
}
...
function safeDecodeURI (val) {
  try {
    ...
    if (e.constructor == URIError) {
      warn(false, "Tried to decode an invalid URI: " + JSON.stringify(val) + " " + e.stack);
    }
...
class Url {
  ...
  static fromQueryString(query) {
    ...
    // Query strings are visitor-controlled, so keys that collide with
    // Object.prototype members (__proto__, constructor, toString, ...) must
    // become plain own properties: defineProperty assignment plus an
    // own-property read guard keeps them from reaching the prototype.
    const setOwn = (key, value) => {
    ...
    keyValuePairs.forEach((pair) => {
      // split on the first '=' only; later '=' characters belong to the value
      // (e.g. base64 padding in ?token=abc==)
      const eq = pair.indexOf('=');
      let key = safeDecodeQueryComponent(eq === -1 ? pair : pair.slice(0, eq));
      const value = eq === -1 ? undefined : safeDecodeQueryComponent(pair.slice(eq + 1));

      // A key that fails to decode (e.g. "?%ZZ=1") must not fall through
      // and create a property literally named "undefined"
