...
RouteController.prototype._runRoute = function (route, url, done) {
  ...
  // If an async action is still in flight, defer the never-rendered warning
  // and the after hooks until the dispatch fully settles - the action gets
  // its chance to render first (mirrors the server _runRoute).
  const finishRun = () => {
  ...
  if (this._pendingAsync)
    this._onAsyncDrain.push(finishRun);
  else
    finishRun();
};
