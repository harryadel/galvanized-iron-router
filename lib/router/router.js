...
class Router {
  ...
  _globalHooks;
  /** @type {Array & { _byPath?: Object<string, Route>, _byName?: Object<string, Route> }} */
  routes;
  ...
  _scrollToHash;
  /**
   * Global hook helper, one of the methods attached to the prototype from
   * Router.HOOK_TYPES at module load. Declared here for type checking only
   * (the constructor returns a separate callable, so class fields never
   * reach instances).
   * @type {(hook: Function|string, options?: Object) => void}
   */
  onBeforeAction;
  ...
  constructor(options) {
    ...
    // name => route map; prototype-less so route names can't collide with
    // Object or Array members (see the route() legacy alias note)
    routerFn.routes._byName = Object.create(null);
  ...
  */
  ...
  route(path, fn, opts) {
    ...
    if (typeof handler.name === 'string') {
      this.routes._byName[handler.name] = route;
      // Legacy alias (Router.routes.someRouteName). Skipped when the name
      // would shadow an Array member (a route at /push or /length must not
      // overwrite the array's methods); use findRouteByName for lookups.
      if (!(handler.name in this.routes))
  ...
  findRouteByName(name) {
    return this.routes._byName[name];
  }
  ...
  registerController(nameOrController, controller) {
    ...
    if (typeof nameOrController === 'function') {
      ...
      // Only an own _name identifies this controller; an inherited one
      // belongs to a parent and would register the child under the parent's
      // key. controller.name is a fallback only - production minifiers
      // mangle class binding names, so prefer the object-map form of
      // registerControllers or a static _name.
      name = (Object.prototype.hasOwnProperty.call(controller, '_name') && controller._name) ||
        controller.name;
      if (!name) {
    ...
    warn(!(this._controllers[name] && this._controllers[name] !== controller),
      "A controller named " + JSON.stringify(name) + " is already registered and is being overwritten. " +
      "If this happens only in production builds, the minifier has probably mangled your class names - " +
      "register controllers with the object-map form, e.g. Router.registerControllers({MyController}).");
...
Router.HOOK_TYPES.forEach((type) => {
  Router.prototype[type] = function (hook, options) {
    // addHook returns the router, keeping these chainable
    return this.addHook(type, hook, options);
  };
});
