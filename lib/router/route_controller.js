...
class RouteController extends Controller {
  ...
  /** @type {Function} */
  next;
  /** @type {number} count of in-flight async handlers, managed by MiddlewareStack.dispatch */
  _pendingAsync;
  /** @type {Array<Function>} callbacks run when _pendingAsync drains to zero */
  _onAsyncDrain;
  /** @type {boolean} */
  ...
  /**
   * Suppresses the Controller base constructor's init() call; this
   * constructor calls init() above, after route/params/data are in place.
   * See Controller#_deferInit for why this is a getter, not a class field.
   * @returns {boolean}
   */
  get _deferInit() { return true; }
  ...
  _collectHooks(/* hook1, alias1, ... */) {
    ...
    const collectInheritedHooks = (ctor, hookName) => {
      // Walk the real prototype chain rather than __super__: for a native
      // `class Child extends Base` chain, Child.__super__ resolves through
      // static inheritance to an ancestor's value, silently skipping Base's
      // hooks. The prototype chain covers extend() products and native
      // classes uniformly.
      const perLevel = [];
      let proto = ctor.prototype;
      while (proto && proto !== Object.prototype) {
        if (Object.prototype.hasOwnProperty.call(proto, hookName))
          perLevel.push(proto[hookName]);
        proto = Object.getPrototypeOf(proto);
      }

      // super-first ordering, matching the old __super__ recursion
      let hooks = [];
      for (let i = perLevel.length - 1; i >= 0; i--)
        hooks = hooks.concat(getHookValues(perLevel[i]));
      return hooks;
    };
