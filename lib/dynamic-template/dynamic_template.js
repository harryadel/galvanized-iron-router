...
class DynamicTemplate {
  ...
  /**
   * Detect Blaze major version when available. Memoized: it is called from
   * hot view-traversal paths and the Blaze version cannot change at runtime.
   */
  static _isBlaze2() {
    if (DynamicTemplate._isBlaze2Memo === undefined) {
      ...
      if (!version || typeof version !== 'string') {
        DynamicTemplate._isBlaze2Memo = false;
      } else {
        const major = parseInt(version.split('.')[0], 10);
        DynamicTemplate._isBlaze2Memo = Number.isFinite(major) && major < 3;
      }
    }
