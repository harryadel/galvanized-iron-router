...
Iron.utils.extend = function (Parent, props) {
  ...
  // Deep-copy own enumerable statics (event maps, helpers) so child
  // mutations don't write through to the parent. Other statics are
  // inherited through the class chain. __super__ and _name describe the
  // parent specifically, so they must not be copied down.
  for (const key in Parent) {
    if (Object.prototype.hasOwnProperty.call(Parent, key) &&
        key !== '__super__' && key !== '_name')
      ctor[key] = EJSON.clone(Parent[key]);
  }
