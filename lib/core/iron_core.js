...
/**
 * Copy a static property value for inheritance. Only plain objects and
 * arrays are shallow-copied, so that top-level child mutations of mutable
 * collection statics (helpers, event maps) don't write through to the
 * parent; nested values are shared. Everything else - RegExp, Map, Set,
 * prototyped instances - is passed by reference. (EJSON.clone was used
 * previously and silently mangled such non-EJSON objects into {}.)
 */
const copyStaticValue = function (value) {
...
Iron.utils.inherits = function (Child, Parent, props) {
  ...
  for (const key in Parent) {
    if (Object.prototype.hasOwnProperty.call(Parent, key))
      Child[key] = copyStaticValue(Parent[key]);
  }
...
Iron.utils.extend = function (Parent, props) {
  ...
  const hasCustomCtor = Object.prototype.hasOwnProperty.call(props, 'constructor') &&
    typeof props.constructor === 'function';

  // A genuine subclass behaves the same whether Parent is an ES6 class, a
  // previous extend() result, or a plain constructor function, and it keeps
  // working at any chain depth. (The previous implementation returned a
  // wrapper function, so a second-level extend took an ES5 path that
  // discarded the instance the wrapper built - grandchild instances never
  // received the base constructor's initialization.)
  //
  // A custom `constructor` prop runs as a post-construction initializer: the
  // parent chain has already run via super(), so it must not invoke
  // Parent.__super__.constructor itself (with an ES6 class ancestor that
  // call throws, exactly as it did before this implementation).
  const ParentCtor = /** @type {new (...args: any[]) => any} */ (
    /** @type {unknown} */ (Parent));

  /** @type {Function & {_name?: string, __super__?: Object}} */
  const ctor = class extends ParentCtor {
    constructor(...args) {
      super(...args);
      if (hasCustomCtor)
  ...
  // Copy own enumerable statics (event maps, helpers) so top-level child
  // mutations don't write through to the parent; exotic values (RegExp,
  // Map, prototyped instances) pass by reference with their types intact.
  // Other statics are inherited through the class chain. __super__ and
  // _name describe the parent specifically, so they must not be copied down.
  for (const key in Parent) {
    if (Object.prototype.hasOwnProperty.call(Parent, key) &&
        key !== '__super__' && key !== '_name')
      ctor[key] = copyStaticValue(Parent[key]);
  }

  ctor.__super__ = Parent.prototype;

  // Copy prototype props, preserving the class's own constructor reference.
