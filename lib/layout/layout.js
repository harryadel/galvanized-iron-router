...
class Layout extends DynamicTemplate {
  ...
  _runRegionHooks(name, regionView, regionDynamicTemplate) {
    ...
    for (let i = 0; i < hooks.length; i++) {
      const hook = hooks[i];
      // keep the "thisArg" pointing to the view; the callback receives the
      // region name, the region's dynamic template, and the layout
      hook.call(regionView, regionDynamicTemplate._region, regionDynamicTemplate, this);
    }
  }
}
...
if (typeof Template !== 'undefined') {
  ...
  UI.registerHelper('contentFor', new Template('contentFor', function () {
    ...
    // A reactive region= argument can change on rerun. Clear the region this
    // block previously populated so the content doesn't linger in both.
    if (view._contentForRegion && view._contentForRegion !== region)
      layout.clear(view._contentForRegion);
    view._contentForRegion = region;
