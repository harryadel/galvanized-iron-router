...
const mergeStaticInheritedObjectProperty = function (ctor, prop) {
  ...
  // Find the parent constructor. Only an *own* __super__ identifies this
  // constructor's parent - an inherited one belongs to an ancestor and
  // would skip every native `class X extends Y` level in between. Native
  // class chains are walked via Object.getPrototypeOf instead.
  let parent;
  if (Object.prototype.hasOwnProperty.call(ctor, '__super__') && ctor.__super__)
    parent = ctor.__super__.constructor;
  else
    parent = Object.getPrototypeOf(ctor);

  if (typeof parent === 'function' && parent !== Function.prototype)
    Object.assign(merge, mergeStaticInheritedObjectProperty(parent, prop));
...
class Controller {
  ...
  /**
   * Subclasses whose constructors do their own setup after super() (e.g.
   * RouteController setting route/params/data) override this to return true
   * and call init() themselves, so init() runs exactly once, on a fully
   * constructed instance. A getter rather than a class field: a field would
   * be initialized to undefined during super() and shadow the override.
   * @returns {boolean}
   */
  get _deferInit() { return false; }
  ...
  static helpers(helpers) {
    // Copy-on-write: for a native `class X extends Controller`, _helpers
    // resolves to the parent's object through static inheritance, and
    // assigning into it would leak this subclass's helpers to the parent
    // and every sibling.
    if (!Object.prototype.hasOwnProperty.call(this, '_helpers'))
      this._helpers = Object.assign({}, this._helpers);
